...
日期:2019/10/05
功能:
  获取网页ajax,逆向工程解析u3m8地址  然后通过多线程根据m3u8 url地址下载视频
版本:3.0
'''


class FileCombineTool:
    def __init__(self, base_path, target_path, file_list):
    ...
    def combine(self):
        ...
        file_path = self.target_path + '.mp4'
...
class M3u8DownloadTool:
    ...
    def __call__(self, max_thread_num):
        ...
        target_file_path = self.target_path + '/' + self.title
        if  not os.path.exists(target_file_path):
            os.mkdir(target_file_path)
        FileCombineTool(self.base_path, target_file_path, ts_url_list)


def js_parse(video_url):
    urllib3.disable_warnings()
    response = requests.get(video_url)
    js_url = re.findall('mac_url=unescape\(\'(.*?)\'\);', response.text, re.S)[0]
    v_title = re.findall('mac_name=\'(.*?)\',', response.text, re.S)[0]
    url_1 = js_url.replace('%2F', '/').replace('%3A', ':')
    index_1 = url_1.index('http')
    url_2 = url_1[index_1 + 4:]
    index_2 = url_2.index('http')
    index_m3u8 = url_2[index_2:]
    response = requests.get(index_m3u8)
    index_end = index_m3u8.index('.com') + 4
    head_url = index_m3u8[:index_end]
    m3u8_url = head_url + response.text.split('\n')[-1]
    return m3u8_url, v_title


def get_video_link_list(page_url):
    address_head_url = 'http://www.yhdzx.com'
    page_response = requests.get(page_url).text
    page_str = etree.HTML(page_response).xpath('//div[@class="panel-max"]')[0]
    link_list = page_str.xpath('./ul/li/a/@href')
    video_link_list = []
    for v_url in link_list:
        urllib3.disable_warnings()
        play_response = requests.get(address_head_url+v_url).text
        html_str = etree.HTML(play_response)
        video_url = address_head_url + html_str.xpath('//*[@id="detail-content"]/ul[1]/li/a/@href')[0]
        m3u8_url, v_title = js_parse(video_url)
        video_link_list.append((m3u8_url, v_title))
        m3u8_download = M3u8DownloadTool(m3u8_url, v_title)
        m3u8_download(30)
    return video_link_list


if __name__ == '__main__':
    print(get_video_link_list('http://www.yhdzx.com/?m=vod-type-id-4-pg-1.html'))
